    ...
    : products.map((product, index) => (
        <Polaris.IndexTable.Row key={product.id}>
          ...
          <Polaris.IndexTable.Cell>
            {context.updatedProductMargins.find(
              ...
              <Polaris.TextField
                ...
                suffix="%"
                value={
                  margins.length === products.length
                    ? margins[index].margin * 100
                    : 0
                }
              />
            )}
          ...
          <Polaris.IndexTable.Cell>
            <Polaris.TextContainer>
              {margins.length === products.length
                ? (margins[index].margin * product.variants[0].price).toFixed(2)
                : 0}
              {currencyCodeToSymbol(product.variants[0].currencyCode)}
            </Polaris.TextContainer>
          </Polaris.IndexTable.Cell>
